...
const List<ChangelogEntry> kChangelogEntries = [
  ChangelogEntry(
    ...
    sections: [
      ...
      ChangelogSection(
        title: 'Extended Volume (up to 200%)',
        bullets: [
          'Volume boost up to **200%** via LoudnessEnhancer, with enable toggle in Audio settings.',
          'Volume preserved across engine recreation; slider reflects boost range.',
          'Volume authority refactor — hardware vs software routing via `isHardwareVolumeAuthority`.',
        ],
      ),
      ChangelogSection(
        title: 'Parametric EQ & Audio',
        bullets: [
          'Graphic EQ → **parametric EQ**: variable band types with RBJ coefficients, animated band add/remove.',
          'EQ now survives engine recreation and reapplies on audio session changes; no more reset glitches.',
          'EQ correctly bypassed on bit-perfect output.',
      ...
      ChangelogSection(
        title: 'Bluetooth',
        bullets: [
          'Hi-Res Direct no longer forces the `dapInternalHighRes` engine on BT routes — fixes volume loss and 5-10s dropouts over A2DP.',
